...
if(isset($_POST['m_submit'])) {
	if($_POST['m_password'] === $_POST['m_password2']) {
		...
		$res = mysql_query("SELECT `$game_id` FROM `$game_user` WHERE `$game_login`='{$_POST['m_login']}' OR `$game_name`='{$_POST['m_name']}'");
		echo mysql_error();
		if(@mysql_num_rows($res) < 1) {
			mysql_query("INSERT INTO `user` (`$game_login`, `$game_password`, `$game_email`, `$game_name`, `class`, `level`, `exp`, `hp`) VALUES ('{$_POST['m_login']}', '".md5($_POST['m_password'])."', '{$_POST['m_email']}', '{$_POST['m_name']}', 'a', 0, 0, 99)");
			echo mysql_error();
